...
second_stage <- lm(beta_CA ~ crisis + Clus_inf_1 + Clus_inf_2
                   + rgdp + rgdp_growth + UNRATE + EXPINF1YR + lag_GB + EU +
                     JP + VIXCLS + WTISPLC + GSCPI, 
                   data = df_08)

second_stage2 <- lm(beta_GB ~ crisis + Clus_inf_1 + Clus_inf_2
...
second_stage2 <- lm(beta_EU ~ crisis + Clus_inf_1 + Clus_inf_2
...
second_stage2 <- lm(beta_FR ~ crisis + Clus_inf_1 + Clus_inf_2
                    + rgdp + rgdp_growth + UNRATE + EXPINF1YR + CA + lag_GB +
                      JP + VIXCLS + WTISPLC + GSCPI, 
                    data = df_covid)
...
second_stage <- lm(beta_CA ~ ShiftCrisis + Clus_inf_1 + Clus_inf_2
                   + rgdp + rgdp_growth + UNRATE + EXPINF1YR + lag_GB + EU +
                     JP + VIXCLS + WTISPLC + GSCPI, 
                   data = df_08)
